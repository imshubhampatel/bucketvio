        ...
        try {
            await User.findOne({ email: req.body.email }, async (err, user) => {
                ...
                if (!user) {
                    ...
                    let token = await jwt.sign({ id: user._id }, process.env.ACCESS_TOKEN_SECRET, { expiresIn: "1d" });
                    console.log(token);
                    if (token) {
        ...
        try {
            ...
            await User.findOne({ email }, async (err, user) => {
                ...
                // if user found then 
                    ...
                    if (isMatch) {
                        console.log(user._id);
                        var token = await jwt.sign({ id: user._id }, process.env.ACCESS_TOKEN_SECRET, { expiresIn: "1d" });
                        return res.status(200).json({ message: "login successully", data: { success: true, token: token, userDetails: user } });
                    }
                }
                else {
